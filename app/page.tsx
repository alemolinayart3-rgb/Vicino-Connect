...
function Profile({ role, onLogout }: { role: Role; onLogout: () => void }) {
  ...
  return <div className="page-content"><div className="page-heading"><p className="eyebrow">TU ESPACIO</p><h1>Perfil</h1></div><section className="profile-settings"><div className={`avatar xl ${data.color}`}>{data.initial}</div><div><h2>{data.name}</h2><p>{data.subtitle}</p>{data.license&&<small>{data.license}</small>}</div><button className="secondary" onClick={()=>setEditing(true)}>Editar perfil</button></section>{role!=='Paciente'&&<section className="professional-identity"><span className={`role-badge ${role==='Psicólogo'?'sage':'blue'}`}>{role}</span><div><strong>Perfil profesional verificado</strong><p>{role==='Psicólogo'?'Acceso al espacio psicológico y procesos terapéuticos.':'Acceso al espacio médico, indicaciones y medicación.'}</p></div></section>}<div className="settings-grid">{data.items.map((x,i)=><button className="setting" key={x}><span>{['○','◇','◌','?'][i]}</span><strong>{x}</strong><i>→</i></button>)}</div><button className="logout" onClick={onLogout}>Cerrar sesión</button>{editing&&<div className="modal-backdrop" onClick={()=>setEditing(false)}><section className="modal" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setEditing(false)}>×</button><p className="eyebrow">EDITAR PERFIL</p><h2>{data.name}</h2><label className="edit-field">Nombre completo<input defaultValue={data.name}/></label><label className="edit-field">Rol profesional<input value={data.subtitle} readOnly/></label>{data.license&&<label className="edit-field">Cédula profesional<input defaultValue={data.license.replace('Cédula profesional · ','')}/></label>}<button className="primary" onClick={()=>setEditing(false)}>Guardar cambios</button></section></div>}</div>
}
