...
function ProfessionalHome({ role, onMessages }: { role: Exclude<Role,'Paciente'>; onMessages: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => { link.remove(); screen.remove(); patientSummaryLink?.remove(); patientSummaryScreen?.remove(); aiSummaryLinks.forEach(item=>item.remove()); aiSummaryScreens.forEach(item=>item.remove()); };
  }, [role]);
