...
function ProfessionalWorkspace({role,section}:{role:Role;section:'Seguimiento'|'Recursos'}){
 ...
 const [resource,setResource]=useState<{title:string;kind:string}|null>(null),[patients,setPatients]=useState<AssignedPatient[]>([]),[patientId,setPatientId]=useState(''),[instructions,setInstructions]=useState(''),[frequency,setFrequency]=useState('once'),[saving,setSaving]=useState(false),[message,setMessage]=useState('');
 const openResource=async(title:string,kind:string)=>{setResource({title,kind});setMessage('Cargando pacientes…');const {data,error}=await createClient().rpc('my_assigned_patients');const active=((data as AssignedPatient[])||[]).filter(item=>item.status==='active');setPatients(active);setPatientId(active[0]?.assignment_id||'');setMessage(error?error.message:active.length?'':'Aún no tienes pacientes activos para asignar este recurso.')};
 const assignResource=async()=>{if(!resource||!patientId)return;setSaving(true);setMessage('Guardando asignación…');const client=createClient();const {data:{user}}=await client.auth.getUser();const {error}=await client.from('care_plan_items').insert({assignment_id:patientId,created_by:user?.id,kind:resource.kind,title:resource.title,instructions:instructions.trim()||null,frequency});setSaving(false);if(error){setMessage(error.message);return}setMessage('Recurso asignado correctamente. Ya aparecerá en el espacio del paciente.');setTimeout(()=>{setResource(null);setInstructions('');setFrequency('once');setMessage('')},1100)};
 ...
 const resources=medical?[['Indicaciones','Plantillas para comunicar indicaciones médicas.','activity'],['Seguimiento de tolerancia','Registros que el paciente puede responder.','check_in'],['Revisión de tratamiento','Recordatorios para próximas consultas.','activity']]:[['Actividades acordadas','Ejercicios para acompañar entre sesiones.','activity'],['Registros breves','Check-ins diarios o semanales.','check_in'],['Cuestionarios','Instrumentos y formularios reutilizables.','check_in'],['Material psicoeducativo','Recursos de lectura y práctica.','activity']];
 return <div className="page-content professional-workspace"><div className="page-heading"><p className="eyebrow">{medical?'GESTIÓN CLÍNICA':'BIBLIOTECA DE ACOMPAÑAMIENTO'}</p><h1>{medical?'Tratamientos':'Recursos'}</h1><p>{medical?'Organiza indicaciones y seguimientos con límites de acceso claros.':'Prepara recursos reutilizables y asígnalos desde el perfil de cada paciente.'}</p></div><div className="resource-grid">{resources.map(([title,text,kind])=><section className="panel resource-card" key={title}><span>◇</span><h2>{title}</h2><p>{text}</p><button className="secondary" onClick={()=>openResource(title,kind)}>Asignar a paciente</button></section>)}</div>{resource&&<div className="modal-backdrop" onClick={()=>setResource(null)}><section className="modal resource-assignment-modal" role="dialog" aria-modal="true" aria-label={`Asignar ${resource.title}`} onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setResource(null)}>×</button><p className="eyebrow">ASIGNAR RECURSO</p><h2>{resource.title}</h2><p>Elige a quién se lo compartirás y añade indicaciones claras.</p>{patients.length>0&&<><label className="edit-field">Paciente<select value={patientId} onChange={e=>setPatientId(e.target.value)}>{patients.map(patient=><option value={patient.assignment_id} key={patient.assignment_id}>{patient.full_name}</option>)}</select></label><label className="edit-field">Frecuencia<select value={frequency} onChange={e=>setFrequency(e.target.value)}><option value="once">Una vez</option><option value="daily">Diario</option><option value="weekly">Semanal</option></select></label><label className="edit-field">Indicaciones, opcional<textarea value={instructions} onChange={e=>setInstructions(e.target.value)} placeholder="Explica brevemente cómo utilizar este recurso."/></label></>}{message&&<p className="invite-feedback" role="status">{message}</p>}<button className="primary" disabled={saving||!patientId} onClick={assignResource}>{saving?'Asignando…':'Confirmar asignación'}</button></section></div>}</div>;
}
...
function FreshPatientView({ tab, name }: { tab: Tab; name: string }) {
  ...
  const [careItems,setCareItems]=useState<{id:string;kind:string;title:string;instructions:string|null;frequency:string;created_at:string}[]>([]);
