...
import ProfessionalResourceCreator from "@/app/components/ProfessionalResourceCreator";
import ProfessionalResponses from "@/app/components/ProfessionalResponses";
...
function ProfessionalWorkspace({role,section}:{role:Role;section:'Seguimiento'|'Recursos'}){
 const medical=role==='Psiquiatra';
 return section==='Recursos'?<ProfessionalResourceCreator medical={medical}/>:<ProfessionalResponses medical={medical}/>;
}
...
function FreshPatientView({ tab, name }: { tab: Tab; name: string }) {
  ...
  const [careItems,setCareItems]=useState<PatientCareItem[]>([]),[selectedCareItem,setSelectedCareItem]=useState<PatientCareItem|null>(null),[completedCareIds,setCompletedCareIds]=useState<string[]>([]);
  ...
  useEffect(()=>{(async()=>{const client=createClient();const {data:{user}}=await client.auth.getUser();const today=new Date().toISOString().slice(0,10);const [{data:team},{data:items},{data:profile},{data:responses}]=await Promise.all([client.rpc('my_care_team'),client.from('care_plan_items').select('id,kind,resource_type,title,instructions,content,frequency,created_at').neq('kind','private_note').order('created_at',{ascending:false}),client.from('profiles').select('phone,birth_date').eq('id',user?.id||'').maybeSingle(),client.from('care_item_responses').select('care_item_id').eq('patient_id',user?.id||'').eq('response_date',today).eq('status','submitted')]);setCareTeam((team as typeof careTeam)||[]);setCareItems((items as typeof careItems)||[]);setCompletedCareIds((responses||[]).map(item=>item.care_item_id));setContact({phone:profile?.phone||'',birthDate:profile?.birth_date||''});setTeamLoading(false)})()},[]);
