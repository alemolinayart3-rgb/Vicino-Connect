...
import PatientResourceModal, { type PatientCareItem } from "@/app/components/PatientResourceModal";

type Screen = "login" | "onboarding" | "profileSetup" | "app";
type Tab = "Inicio" | "Agenda" | "Pacientes" | "Proceso" | "Actividades" | "Equipo" | "Seguimiento" | "Recursos" | "Mensajes" | "Perfil";
...
function ActivePatientsWorkspace(){
 ...
 const emptyForm={type:'activity',title:'',instructions:'',content:'',frequency:'once'};
 ...
 const save=async()=>{
  if(!selected||form.title.trim().length<3){setMessage('Escribe un título breve.');return}
  if(form.type==='questionnaire'&&!form.content.trim()){setMessage('Añade al menos una pregunta.');return}
  if(form.type==='educational_material'&&!form.content.trim()){setMessage('Añade el enlace o referencia del material.');return}
  setMessage('Guardando…');
  const client=createClient();const {data:{user}}=await client.auth.getUser();
  const kind=form.type==='private_note'?'private_note':form.type==='check_in'?'check_in':'activity';
  const content=form.type==='questionnaire'?{questions:form.content.split('\n').map(item=>item.trim()).filter(Boolean)}:form.type==='educational_material'?{url:form.content.trim()}:form.type==='check_in'?{prompt:form.content.trim()}:{};
  const {error}=await client.from('care_plan_items').insert({assignment_id:selected.assignment_id,created_by:user?.id,kind,resource_type:form.type,title:form.title.trim(),instructions:form.instructions.trim()||null,content,frequency:form.type==='private_note'?'once':form.frequency});
  if(error){setMessage(error.message);return}setMessage('Seguimiento asignado correctamente. Ya aparecerá en el espacio del paciente.');setForm(emptyForm);setTimeout(()=>setSelected(null),900)
 };
 if(!target||!patients.length)return null;
 return createPortal(<section className="panel active-patients" onClick={event=>event.stopPropagation()}><div className="section-title"><div><p className="eyebrow">VÍNCULOS CONFIRMADOS</p><h2>Pacientes activos</h2></div><span>{patients.filter(x=>x.status==='active').length} activos</span></div><div className="active-patient-list">{patients.map(patient=><article key={patient.assignment_id}><div className="avatar sage">{patient.full_name.split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase()}</div><div><strong>{patient.full_name}</strong><small>{patient.status==='active'?'Acompañamiento activo':'Vínculo suspendido'}</small></div><i className={`care-link-status ${patient.status}`}>{patient.status==='active'?'Activo':'Suspendido'}</i><button className="secondary" disabled={patient.status!=='active'} onClick={()=>{setSelected(patient);setForm(emptyForm);setMessage('')}}>Añadir seguimiento</button></article>)}</div>{selected&&<div className="follow-up-composer"><div className="section-title"><div><p className="eyebrow">PARA {selected.full_name.toUpperCase()}</p><h3>Asignar acompañamiento</h3></div><button aria-label="Cerrar" onClick={()=>setSelected(null)}>×</button></div><div className="resource-type-picker">{[['activity','Ejercicio'],['questionnaire','Encuesta'],['check_in','Registro breve'],['educational_material','Material'],['private_note','Nota privada']].map(([value,label])=><button className={form.type===value?'selected':''} onClick={()=>setForm({...form,type:value,content:''})} key={value}>{label}</button>)}</div><div className="follow-up-fields"><label>Frecuencia<select disabled={form.type==='private_note'||form.type==='educational_material'} value={form.type==='private_note'||form.type==='educational_material'?'once':form.frequency} onChange={e=>setForm({...form,frequency:e.target.value})}><option value="once">Una vez</option><option value="daily">Diario</option><option value="weekly">Semanal</option></select></label><label>Título<input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="Ej. Registro de sueño"/></label></div>{form.type==='questionnaire'&&<label>Preguntas, una por línea<textarea value={form.content} onChange={e=>setForm({...form,content:e.target.value})} placeholder={'¿Cómo te sentiste hoy?\n¿Qué te ayudó durante el día?'}/></label>}{form.type==='educational_material'&&<label>Enlace o referencia<input value={form.content} onChange={e=>setForm({...form,content:e.target.value})} placeholder="https://..."/></label>}{form.type==='check_in'&&<label>Pregunta del registro<input value={form.content} onChange={e=>setForm({...form,content:e.target.value})} placeholder="¿Cómo estuvo tu nivel de energía hoy?"/></label>}<label>Indicaciones<textarea value={form.instructions} onChange={e=>setForm({...form,instructions:e.target.value})} placeholder="Describe el acompañamiento con un tono amable."/></label>{form.type==='private_note'&&<p className="privacy-hint">Esta nota es clínica y privada; no aparecerá en el espacio del paciente.</p>}{message&&<p className="invite-feedback" role="status">{message}</p>}<button className="primary" onClick={save}>Asignar a {selected.full_name}</button></div>}</section>,target);
}
...
type Appointment={id:string;assignment_id:string;patient_id:string;professional_id?:string;starts_at:string;duration_minutes:number;status:string;notes:string|null};

type AppointmentReceiptData={title:string;detail:string;personLabel:string;personName:string;startsAt:string;duration:number};
function AppointmentReceipt({data,onClose}:{data:AppointmentReceiptData;onClose:()=>void}){
 const when=new Date(data.startsAt);
 return createPortal(<div className="modal-backdrop appointment-confirmation-backdrop" onClick={onClose}><section className="modal appointment-confirmation" role="dialog" aria-modal="true" aria-label={data.title} onClick={event=>event.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><span className="confirmation-mark">✓</span><p className="eyebrow">CITA REGISTRADA</p><h2>{data.title}</h2><p className="confirmation-copy">{data.detail}</p><div className="confirmation-summary"><span><small>{data.personLabel}</small><strong>{data.personName}</strong></span><span><small>Fecha</small><strong>{when.toLocaleDateString('es-MX',{day:'numeric',month:'long',year:'numeric'})}</strong></span><span><small>Hora</small><strong>{when.toLocaleTimeString('es-MX',{hour:'numeric',minute:'2-digit'})}</strong></span><span><small>Duración</small><strong>{data.duration} minutos</strong></span></div><button className="primary" onClick={onClose}>Entendido</button></section></div>,document.body);
}

function ProfessionalAppointmentReceipt(){
 const [data,setData]=useState<AppointmentReceiptData|null>(null);
 useEffect(()=>{const show=(event:Event)=>setData((event as CustomEvent<AppointmentReceiptData>).detail);window.addEventListener('vicino:appointment-created',show);return()=>window.removeEventListener('vicino:appointment-created',show)},[]);
 return data?<AppointmentReceipt data={data} onClose={()=>setData(null)}/>:null;
}

function PatientAppointments({careTeam}:{careTeam:CareProfessional[]}){
 const [appointments,setAppointments]=useState<Appointment[]>([]),[receipt,setReceipt]=useState<AppointmentReceiptData|null>(null),[message,setMessage]=useState('');
 const load=async()=>{const {data}=await createClient().from('appointments').select('id,assignment_id,patient_id,professional_id,starts_at,duration_minutes,status,notes').in('status',['pending_patient','confirmed','scheduled']).order('starts_at');setAppointments((data as Appointment[])||[])};
 useEffect(()=>{load()},[]);
 const professionalName=(appointment:Appointment)=>careTeam.find(person=>person.professional_id===appointment.professional_id)?.full_name||'Tu profesional';
 const confirm=async(appointment:Appointment)=>{setMessage('Confirmando…');const {error}=await createClient().from('appointments').update({status:'confirmed',confirmed_at:new Date().toISOString()}).eq('id',appointment.id);if(error){setMessage(error.message);return}const person=professionalName(appointment);setMessage('');setReceipt({title:'Tu cita está confirmada',detail:'Gracias por confirmar. Este encuentro ya forma parte de tu agenda de Vicino.',personLabel:'Profesional',personName:person,startsAt:appointment.starts_at,duration:appointment.duration_minutes});await load()};
 if(appointments.length===0)return <section className="panel empty-state"><p className="eyebrow">PRIMEROS PASOS</p><h2>Aún no tienes sesiones programadas</h2><p>Cuando un profesional programe un encuentro, aparecerá aquí para que puedas confirmarlo.</p></section>;
 return <>{receipt&&<AppointmentReceipt data={receipt} onClose={()=>setReceipt(null)}/>}<section className="panel patient-appointments"><div className="section-title"><div><p className="eyebrow">TU AGENDA</p><h2>Próximos encuentros</h2></div><span>{appointments.length}</span></div>{appointments.map(appointment=><article key={appointment.id}><div className="appointment-date"><strong>{new Date(appointment.starts_at).getDate()}</strong><small>{new Date(appointment.starts_at).toLocaleDateString('es-MX',{month:'short'}).replace('.','').toUpperCase()}</small></div><div><strong>{professionalName(appointment)}</strong><p>{new Date(appointment.starts_at).toLocaleString('es-MX',{weekday:'long',day:'numeric',month:'long',hour:'numeric',minute:'2-digit'})} · {appointment.duration_minutes} min</p><small>{appointment.notes||'Encuentro de seguimiento'}</small></div>{appointment.status==='pending_patient'?<button className="primary" onClick={()=>confirm(appointment)}>Confirmar cita</button>:<i>Confirmada</i>}</article>)}{message&&<p className="agenda-feedback" role="status">{message}</p>}</section></>;
}

function TimePicker({value,onChange,slots}:{value:string;onChange:(value:string)=>void;slots:{value:string;label:string}[]}){
 const [open,setOpen]=useState(false);
 const selected=slots.find(slot=>slot.value===value)?.label||'Elegir hora';
 const morning=slots.filter(slot=>Number(slot.value.slice(0,2))<12),afternoon=slots.filter(slot=>Number(slot.value.slice(0,2))>=12);
 return <div className={`vicino-time-picker ${open?'open':''}`}><button type="button" className="time-picker-trigger" aria-haspopup="dialog" aria-expanded={open} onClick={()=>setOpen(!open)}><span className="time-clock">◷</span><strong>{selected}</strong><i>⌄</i></button>{open&&<div className="time-picker-popover" role="dialog" aria-label="Seleccionar horario"><header><div><p className="eyebrow">HORARIO</p><strong>Elige una hora</strong></div><button type="button" aria-label="Cerrar horarios" onClick={()=>setOpen(false)}>×</button></header>{[['Mañana',morning],['Tarde',afternoon]].map(([title,items])=><section key={title as string}><span>{title as string}</span><div>{(items as typeof slots).map(slot=><button type="button" className={slot.value===value?'selected':''} onClick={()=>{onChange(slot.value);setOpen(false)}} key={slot.value}>{slot.label.replace(' a. m.','').replace(' p. m.','')}</button>)}</div></section>)}</div>}</div>;
}
...
function ProfessionalAgenda(){
 ...
 useEffect(()=>{load()},[]);
 const createAppointment=async()=>{const selected=patients.find(x=>x.assignment_id===patientId);if(!selected||!date||!time)return;setSaving(true);setMessage('Agendando…');const client=createClient();const {data:{user}}=await client.auth.getUser();const startsAt=new Date(`${date}T${time}:00`).toISOString();const {error}=await client.from('appointments').insert({assignment_id:selected.assignment_id,patient_id:selected.patient_id,professional_id:user?.id,starts_at:startsAt,duration_minutes:50,status:'pending_patient',notes:notes.trim()||null});setSaving(false);if(error){setMessage(error.message.includes('appointments')?'Falta activar el módulo de confirmación de citas en Supabase. Ejecuta el archivo SQL de confirmaciones.':error.message);return}setMessage('Solicitud enviada. Esperando confirmación del paciente.');window.dispatchEvent(new CustomEvent('vicino:appointment-created',{detail:{title:'Cita creada correctamente',detail:`Tú programaste este encuentro con ${selected.full_name}. Ahora solo falta que el paciente lo confirme.`,personLabel:'Paciente',personName:selected.full_name,startsAt,duration:50}}));setNotes('');await load()};
 ...
 const days=Array.from({length:31},(_,i)=>i+1),eventDays=new Set(appointments.filter(x=>['pending_patient','confirmed','scheduled'].includes(x.status)).map(x=>new Date(x.starts_at).getDate()));
...
function ProfessionalWorkspace({role,section}:{role:Role;section:'Seguimiento'|'Recursos'}){
 ...
 const [resource,setResource]=useState<{title:string;kind:string;resourceType:string}|null>(null),[patients,setPatients]=useState<AssignedPatient[]>([]),[patientId,setPatientId]=useState(''),[resourceTitle,setResourceTitle]=useState(''),[instructions,setInstructions]=useState(''),[resourceContent,setResourceContent]=useState(''),[resourceFile,setResourceFile]=useState<File|null>(null),[frequency,setFrequency]=useState('once'),[saving,setSaving]=useState(false),[message,setMessage]=useState('');
 const openResource=async(title:string,kind:string,resourceType:string)=>{setResource({title,kind,resourceType});setResourceTitle(title);setInstructions('');setResourceContent('');setResourceFile(null);setFrequency('once');setMessage('Cargando pacientes…');const {data,error}=await createClient().rpc('my_assigned_patients');const active=((data as AssignedPatient[])||[]).filter(item=>item.status==='active');setPatients(active);setPatientId(active[0]?.assignment_id||'');setMessage(error?error.message:active.length?'':'Aún no tienes pacientes activos para asignar este recurso.')};
 const assignResource=async()=>{if(!resource||!patientId||resourceTitle.trim().length<3)return;setSaving(true);setMessage('Guardando asignación…');const client=createClient();const {data:{user}}=await client.auth.getUser();let fileData:{}={};if(resourceFile){if(resourceFile.size>10485760){setSaving(false);setMessage('El archivo no puede superar 10 MB.');return}const safeName=resourceFile.name.replace(/[^a-zA-Z0-9._-]/g,'-');const filePath=`${patientId}/${crypto.randomUUID()}-${safeName}`;const {error:uploadError}=await client.storage.from('care-resources').upload(filePath,resourceFile);if(uploadError){setSaving(false);setMessage(uploadError.message.includes('Bucket not found')?'Falta activar el módulo de archivos privados en Supabase.':uploadError.message);return}fileData={file_path:filePath,file_name:resourceFile.name,file_type:resourceFile.type}}const content=resource.resourceType==='questionnaire'?{questions:resourceContent.split('\n').map(x=>x.trim()).filter(Boolean),...fileData}:resource.resourceType==='educational_material'?{url:resourceContent.trim(),...fileData}:resource.resourceType==='check_in'?{prompt:resourceContent.trim(),...fileData}:{...fileData};const {error}=await client.from('care_plan_items').insert({assignment_id:patientId,created_by:user?.id,kind:resource.kind,resource_type:resource.resourceType,title:resourceTitle.trim(),instructions:instructions.trim()||null,content,frequency});setSaving(false);if(error){setMessage(error.message.includes("care_plan_items")?'Falta activar el módulo de recursos en Supabase. Ejecuta el archivo SQL de configuración que te compartimos.':error.message);return}setMessage('Recurso asignado correctamente. Ya aparecerá en el espacio del paciente.');setTimeout(()=>{setResource(null);setInstructions('');setResourceContent('');setResourceFile(null);setFrequency('once');setMessage('')},1100)};
...
function FreshPatientView({ tab, name }: { tab: Tab; name: string }) {
  ...
  const [careItems,setCareItems]=useState<PatientCareItem[]>([]),[selectedCareItem,setSelectedCareItem]=useState<PatientCareItem|null>(null);
  ...
  if (tab === 'Proceso') return <div className="page-content"><div className="page-heading"><p className="eyebrow">TU CAMINO</p><h1>Mi proceso</h1><p>Una mirada amable a lo que vas construyendo.</p></div><div className="process-overview"><section className="panel process-now"><p className="eyebrow">DONDE ESTÁS AHORA</p><h2>{careTeam.length?'Tu acompañamiento está en marcha':'Tu espacio está listo para comenzar'}</h2><p>{careTeam.length?'Aquí iremos reuniendo los temas, avances y momentos importantes de tu proceso, sin convertirlos en pendientes.':'Cuando un profesional forme parte de tu equipo, este espacio comenzará a crecer contigo.'}</p></section><section className="panel process-rhythm"><span>○</span><div><p className="eyebrow">A TU RITMO</p><h3>{careItems.length?careItems.length+' recurso'+(careItems.length===1?'':'s')+' para acompañarte':'Sin actividades pendientes'}</h3><p>Lo que tu profesional comparta estará disponible en Actividades.</p></div></section></div></div>;
  if (tab === 'Actividades') return <div className="page-content"><div className="page-heading"><p className="eyebrow">PARA TI</p><h1>Mis actividades</h1><p>Encuestas, ejercicios y materiales compartidos por tu profesional.</p></div>{careItems.length===0?<section className="panel empty-state"><h2>Aún no tienes actividades</h2><p>Cuando tu profesional comparta una encuesta, ejercicio o material, aparecerá aquí.</p></section>:<section className="panel patient-resources"><div className="section-title"><div><p className="eyebrow">COMPARTIDO CONTIGO</p><h2>Actividades y recursos</h2></div><span>{careItems.length}</span></div>{careItems.map(item=><article className="patient-resource-card" key={item.id} onClick={()=>setSelectedCareItem(item)}><span>{item.resource_type==='questionnaire'?'?':item.kind==='check_in'?'○':'◇'}</span><div><h3>{item.title}</h3><p>{item.instructions||item.content?.prompt||'Tu profesional compartió este recurso para acompañar tu proceso.'}</p><div className="patient-resource-meta"><small>{item.frequency==='daily'?'Diario':item.frequency==='weekly'?'Semanal':'Una vez'}</small><button className="secondary" onClick={event=>{event.stopPropagation();setSelectedCareItem(item)}}>{item.resource_type==='educational_material'?'Abrir material':'Responder'}</button></div></div></article>)}</section>}{selectedCareItem&&<PatientResourceModal item={selectedCareItem} onClose={()=>setSelectedCareItem(null)}/>}</div>;
  if (tab === 'Inicio') return <div className="page-content"><div className="page-heading"><p className="eyebrow">TU ESPACIO</p><h1>Hola, {name}</h1><p>Qué gusto acompañarte. Aquí encontrarás tus próximos encuentros.</p></div><PatientAppointments careTeam={careTeam}/><div className="content-grid"><section className="panel empty-state"><h2>Tu proceso comenzará aquí</h2><p>Este espacio se irá construyendo contigo, sin pendientes ni información de ejemplo.</p></section><section className="panel empty-state"><h2>Tu equipo</h2><p>{careTeam.length?`${careTeam.length} profesional${careTeam.length===1?'':'es'} forma${careTeam.length===1?'':'n'} parte de tu red de apoyo.`:'Todavía no hay profesionales vinculados a tu cuenta.'}</p></section></div></div>;
...
export default function App() {
  ...
  return screen === 'login' ? <Login onNext={login}/> : screen === 'profileSetup' ? <ProfileSetup onDone={finishProfileSetup}/> : screen === 'onboarding' ? <Onboarding onDone={finishOnboarding}/> : <><ProfessionalAppointmentReceipt/><Dashboard initialRole={role} profileName={profileName} freshProfile={freshProfile} onLogout={logout}/></>;
}
