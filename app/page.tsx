...
function FreshPatientView({ tab, name }: { tab: Tab; name: string }) {
  ...
  useEffect(()=>{(async()=>{const client=createClient();const [{data:team},{data:items}]=await Promise.all([client.rpc('my_care_team'),client.from('care_plan_items').select('id,kind,title,instructions,frequency,created_at').neq('kind','private_note').order('created_at',{ascending:false})]);setCareTeam((team as typeof careTeam)||[]);setCareItems((items as typeof careItems)||[]);setTeamLoading(false)})()},[]);
