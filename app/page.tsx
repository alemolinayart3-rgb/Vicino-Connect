...
function Team({ onMessage }: { onMessage: (chatId: string) => void }) {
  ...
  return <div className="page-content"><div className="page-heading"><p className="eyebrow">RED DE APOYO</p><h1>Mi equipo</h1><p>Las personas que te acompañan en tu bienestar.</p></div><div className="profile-grid">{people.map(p=><section className="profile-card" key={p.name}><div className={`avatar large ${p.color}`}>{p.initial}</div><h2>{p.name}</h2><p>{p.role}</p><div className="divider"></div><span>{p.desc}</span><div><button className="primary" onClick={()=>onMessage(p.id)}>Mensaje</button><button className="secondary" onClick={()=>setProfile(p.id)}>Ver perfil</button></div></section>)}</div><section className="boundaries"><span>⌁</span><div><h3>Tu privacidad define los límites</h3><p>La información psicológica y médica se mantiene separada. Cada profesional accede únicamente a lo necesario para tu atención.</p></div></section>{selected&&<div className="modal-backdrop" onClick={()=>setProfile(null)}><section className="modal profile-modal" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setProfile(null)}>×</button><div className={`avatar large ${selected.color}`}>{selected.initial}</div><h2>{selected.name}</h2><p className="muted">{selected.role}</p><p>{selected.bio}</p><button className="primary" onClick={()=>onMessage(selected.id)}>Enviar mensaje</button></section></div>}</div>
}
