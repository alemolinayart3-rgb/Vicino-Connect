...
function ProfessionalHome({ role, onMessages }: { role: Exclude<Role,'Paciente'>; onMessages: () => void }) {
  ...
  const patients = [{name:'Ana Rodríguez',initial:'AR',age:'32 años',last:'5 ago. 2026'},{name:'Martín Solís',initial:'MS',age:'41 años',last:'29 jul. 2026'},{name:'Carla Vega',initial:'CV',age:'27 años',last:'7 ago. 2026'}];
  const current = patients.find(item=>item.name===patient);
  ...
  return <div className="page-content professional"><div className="hero-row"><div><p className="eyebrow">PANEL PROFESIONAL</p><h1>Hola, {isPsych ? 'Laura' : 'Diego'}</h1><p>{isPsych ? 'Acompaña el proceso terapéutico de tus pacientes.' : 'Revisa seguimientos e indicaciones médicas.'}</p></div><span className={`role-badge ${isPsych?'sage':'blue'}`}>{role}</span></div><div className="stats"><section><span>Sesiones de hoy</span><strong>{isPsych?'5':'3'}</strong><small>Próxima en 35 min</small></section><section><span>Mensajes pendientes</span><strong>2</strong><button onClick={onMessages}>Revisar mensajes →</button></section><section><span>{isPsych?'Procesos activos':'Seguimientos activos'}</span><strong>{isPsych?'18':'12'}</strong><small>Esta semana</small></section></div><section className="panel professional-list"><div className="section-title"><div><p className="eyebrow">AGENDA DE HOY</p><h2>Próximas consultas</h2></div></div>{patients.map((item,i)=><div className="appointment" key={item.name}><div className={`avatar ${i===0?'sage':''}`}>{item.initial}</div><span><strong>{item.name}</strong><small>{10+i}:00 AM · {isPsych?'Psicoterapia individual':'Seguimiento médico'}</small></span><button className="secondary" onClick={()=>setPatient(item.name)}>Abrir expediente</button></div>)}</section><section className={`boundaries ${isPsych?'psych-boundary':'medical-boundary'}`}><span>⌁</span><div><h3>{isPsych?'Área psicológica protegida':'Área médica protegida'}</h3><p>{isPsych?'Puedes consultar notas terapéuticas, pero no modificar prescripciones médicas.':'Puedes gestionar indicaciones y medicación, sin acceso a notas privadas de psicoterapia.'}</p></div></section></div>;
}
