...
export default function ProfessionalResourceCreator({ medical = false }: { medical?: boolean }) {
  ...
  const open = (type: string, label: string) => {
    ...
    setQuestions([newQuestion()]); setContent(""); setFile(null); setImportedFileName(""); setShowQuestionEditor(false); setMessage("");
  };
  ...
  const importWord = async (wordFile: File) => {
    ...
    try {
      ...
      setImportedFileName(wordFile.name); setShowQuestionEditor(false);
  ...
  return <div className="page-content professional-workspace">
    ...
    {kind && <div className="modal-backdrop" onClick={() => setKind(null)}><section className="modal resource-builder-modal" onClick={(event) => event.stopPropagation()}>
      ...
      {kind === "questionnaire" && (importedFileName ? <div className="imported-questionnaire-summary"><div><span>✓</span><p><strong>{importedFileName}</strong><small>{questions.length} preguntas detectadas · lista para validar</small></p></div><button type="button" onClick={() => { setImportedFileName(""); setQuestions([newQuestion()]); setTitle("Encuestas"); setInstructions(""); setFrequency("once"); setShowQuestionEditor(false); setMessage("Archivo eliminado. Puedes seleccionar otro cuestionario."); }}>Eliminar archivo</button></div> : <label className={`questionnaire-import ${importing ? "loading" : ""}`}><span><strong>{importing ? "Importando Word…" : "Importar cuestionario desde Word"}</strong><small>Vicino detectará el título, las preguntas y sus escalas. Podrás revisar todo antes de asignarlo.</small></span><i>{importing ? "…" : "Subir .docx"}</i><input type="file" accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document" disabled={importing} onChange={(event) => { const selected = event.target.files?.[0]; if (selected) void importWord(selected); event.target.value = ""; }} /></label>)}
      ...
      <label className="builder-file-field">Archivo de apoyo, opcional<input type="file" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg" onChange={(event) => setFile(event.target.files?.[0] || null)} /><small>PDF, Word o imagen · máximo 10 MB</small></label>{file && <div className="selected-support-file"><span><strong>{file.name}</strong><small>{(file.size / 1024 / 1024).toFixed(2)} MB</small></span><button type="button" onClick={() => setFile(null)}>Eliminar archivo</button></div>}
